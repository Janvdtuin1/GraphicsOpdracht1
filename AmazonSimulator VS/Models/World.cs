...
namespace Models {
    public class World : IObservable<Command>, IUpdatable
    {
        ...
        public World() {
            ...
            Shelf ab = CreateShelf(12, 2.15, 9);
            Shelf cd = CreateShelf(15, 2.15, 9);
            Shelf de = CreateShelf(18, 2.15, 9);

            Shelf fg = CreateShelf(12, 2.15, 14);
            Shelf hj = CreateShelf(15, 2.15, 14);
            Shelf kl = CreateShelf(18, 2.15, 14);

            Shelf mn = CreateShelf(12, 2.15, 19);
            Shelf op = CreateShelf(15, 2.15, 19);
            Shelf qr = CreateShelf(18, 2.15, 19);           
        ...
        /// <summary>
        /// Zoekt recursief door de lijst nodes om een node te vinden en returnen waar robots shelves kunnen plaatsen. 
        /// </summary>
        /// <param name="e"></param>
        /// <returns></returns>
        private Node GetEmptyStashLocation(int e)
        {
            int i = e;
            Node n = nodes[i];
            if (n.stash && !(n.checkshelf) && !(n.target))
            {
                n.FlipTarget();               
                return n;
            }
            ...
            {
                return GetEmptyStashLocation(i + 1);
            }
        }
        ...
        //Start het brengTruck event.
        private void BrengTruck()
        {
            brengEvent = true;
            GoalInv = 0;
            t = CreateTruck(-30, 0, 33);
        ...
        {            
            ...
            else if(brengEvent)
            {
                ...
                if(t.checkdes)
                {
                    ...
                    foreach(Robot r in robots)
                    {
                        if(r.destination == GetDropoffLocation(0))
                        {
                            if(!r.justdropped)
                            {
                                r.FlipDropped();
                                Shelf s = CreateShelf(0, (2.4), 0);
                                r.PushShelf(s);
                                t.PlusInv();
                        ...
                        if (!(r.busy))
                        {
                            if (r.destination != GetDropoffLocation(0))
                            {
                                if (!(GoalInv >= t.maxinv))
                                {
                                    r.Changeroute(g.Shortest_path(r.destination, GetDropoffLocation(0)));
                                    GoalInv += 1;
                                }
                            }
                            else
                            {
                                GoalInv += 1;
                                r.Flipbusy();
                                r.Changeroute(g.Shortest_path(r.destination, GetEmptyStashLocation(0)));
                                r.FlipDropped();
                            }
                        }
